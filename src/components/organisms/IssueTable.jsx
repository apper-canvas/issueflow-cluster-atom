    ...
    <table className="w-full">
        <thead className="bg-secondary-50 border-y border-secondary-200">
            <tr>
                ...
                <th className="px-4 py-3 text-left">
                    <span
                        className="text-xs font-semibold text-secondary-700 uppercase tracking-wider">Type
                                      </span>
                </th>
                <th className="px-4 py-3 text-left">
                    <button
                ...
                <th className="px-4 py-3 text-left">
                    <span
                        className="text-xs font-semibold text-secondary-700 uppercase tracking-wider">Assignee
                                      </span>
                </th>
                <th className="px-4 py-3 text-left">
                    <button
        ...
        <tbody className="divide-y divide-secondary-200">
            {issues.map(issue => {
                const isOverdue = issue.dueDate && isPast(new Date(issue.dueDate)) && issue.status !== "resolved" && issue.status !== "closed";
                ...
                return (
                    <tr
                        ...
                        <td className="px-4 py-3">
                            <span
                                className="text-sm font-mono text-secondary-500 group-hover:text-primary transition-colors duration-150">#{issue.Id}
                            </span>
                        </td>
                        <td className="px-4 py-3">
                            <div className="flex items-start gap-2 min-w-[300px]">
                                <div
                                    className={`w-1 h-full rounded-full ${issue.priority === "critical" ? "bg-red-500" : issue.priority === "high" ? "bg-orange-500" : issue.priority === "medium" ? "bg-yellow-500" : "bg-gray-400"}`} />
                                <span
                                    className="text-sm font-medium text-secondary-900 group-hover:text-primary transition-colors duration-150">
                                    {issue.title}
                                </span>
                            </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                                <div
                                    className="w-6 h-6 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0">
                        ...
                        <td className="px-4 py-3">
                            {issue.dueDate ? <div
                                className={`flex items-center gap-1.5 ${isOverdue ? "text-red-600" : "text-secondary-600"}`}>
                                {isOverdue && <ApperIcon name="AlertCircle" size={14} className="flex-shrink-0" />}
